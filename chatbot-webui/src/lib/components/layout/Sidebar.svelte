...
<div
	...
	class="h-screen {show
		? ''
		: '-translate-x-[260px]'}  w-[260px] fixed top-0 left-0 z-40 transition-all duration-300 bg-[#0a0a0a] text-gray-200 shadow-2xl text-sm
        "
>
						...
						<div class=" absolute right-[22px] top-[10px]">
							...
							{:else if chatDeleteId === chat.id}
								<div class="flex items-end space-x-1.5">
									<button
										class=" items-end hover:text-white transition text-sm px-2 py-1 rounded text-red-400"
										on:click={() => {
											deleteChat(chat.id);
										}}
									>
									...
									<button
										class=" items-end hover:text-white transition text-sm px-2 py-1 rounded"
										on:click={() => {
											chatDeleteId = null;
										}}
									>
