<script lang="ts">
	...
	import { chatId, db, chats } from "$lib/stores";
	...
	const handleLike = async (messageId) => {
		if (!history.messages[messageId].feedback) {
			history.messages[messageId].feedback = {};
		}
		history.messages[messageId].feedback.liked = !history.messages[messageId].feedback.liked;
		if (history.messages[messageId].feedback.liked) {
			history.messages[messageId].feedback.disliked = false;
		}

		// Update the chat in the database to save the feedback
		if ($chatId) {
	...
	const handleDislike = async (messageId) => {
		...
		history.messages[messageId].feedback.disliked = !history.messages[messageId].feedback.disliked;
		if (history.messages[messageId].feedback.disliked) {
			history.messages[messageId].feedback.liked = false;
		}
			...
			<div class="flex px-5 mb-3 max-w-3xl mx-auto rounded-lg group {message.role === 'user' ? 'justify-end' : 'justify-start'}">
				<div class="flex w-full {message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}">
					...
					<div class="w-full overflow-hidden">
						<div class=" self-center font-bold mb-0.5 {message.role === 'user' ? 'text-right' : 'text-left'}">
							{#if message.role === "user"}
								You
							{:else}
								Agent <span class=" text-gray-500 text-sm font-medium"
						...
						{:else}
							...
							>
								...
								<textarea
										...
										class=" bg-transparent outline-none w-full resize-none text-right border border-gray-200 dark:border-gray-600 rounded-lg p-2"
								...
								<div class=" mt-2 mb-1 flex justify-end space-x-2 text-sm font-medium">
									...
									{:else}
										<div class="w-full text-right">
								<pre id="user-message" class="text-right">{message.content}</pre>
									...
									<div>
										{#if message?.edit === true}
											<div class=" w-full">
												<textarea
											id="message-edit-{message.id}"
											class=" bg-transparent outline-none w-full resize-none border border-gray-200 dark:border-gray-600 rounded-lg p-2"
											bind:value={history.messages[message.id].editedContent}
											on:input={(e) => {
												e.target.style.height = `${e.target.scrollHeight}px`;
											}}
